# Part 2 – Cognito, Express Backend, and Environment Integration
...
## 🧩 Step 1: Create Cognito User Pools (Dev & Prod)
...
You’ll plug these into both frontend and backend environment files.

---

## ⚙️ Step 2: Create Express Backend for Local Development
...
## 🚀 Step 3: Deploy Express Backend via AWS App Runner Console

We’ll deploy using the **console** for simplicity (no CloudFormation).
...
3. Choose **Source code repository** → Connect your GitHub account.
...
When it’s live, note the **service URL**, e.g.:
```
...
## 🔧 Step 5: Update `deploy-frontend.yml` and redeploy the frontend

Add the new environment variables to your GitHub Actions deploy pipeline:

```yaml
    # ✅ Inject all environment variables for Vite build
