# DiscordClone — PART 1: Frontend Infrastructure (AWS S3 + CloudFront + GitHub CI/CD)
...
## 🧩 1️⃣ Create IAM User and Policies
...
## 🧩 3️⃣ Deploy Infrastructure (S3 + CloudFront)

Save as `infra/frontend-s3-cloudfront.yml`:

```yaml
AWSTemplateFormatVersion: "2010-09-09"
Description: DiscordClone frontend static site hosting (S3 + CloudFront, SPA-ready)
  ...
  FrontendBucket:
    ...
    Properties:
      ...
      PublicAccessBlockConfiguration:
        ...
        BlockPublicPolicy: false   # ✅ allow bucket policy to be applied
      OwnershipControls:
        Rules:
    ...
    Properties:
      DistributionConfig:
        ...
        DefaultCacheBehavior:
          ...
          ForwardedValues:
            QueryString: false
          MinTTL: 0   # ✅ ensures errors aren’t cached incorrectly
...
### 🧠 Executing the Change Set (Manual Review Mode)
...
it means CloudFormation created a **Change Set** (a preview of actions to apply) but didn’t automatically execute it.

---

#### 🔍 What is a Change Set?

A **Change Set** is CloudFormation’s way of showing what it *would* do before actually creating or updating resources.

Normally, `aws cloudformation deploy` executes automatically — but if permissions or flags prevented that (e.g., no `--no-execute-changeset` option), you might need to trigger it manually.

---

#### 🧩 1️⃣ List available Change Sets
...
> 💡 Replace the `--change-set-name` value with the one you copied from the previous step.
...
| Status | Meaning |
|---------|----------|
| `CREATE_IN_PROGRESS` | Stack is building — all good |
| `CREATE_COMPLETE` | Finished successfully |
| `ROLLBACK_IN_PROGRESS` | Error occurred — check stack events |
| `DELETE_FAILED` | Some resources couldn’t be removed (often CloudFront OAI) |
...
That’s your **live frontend endpoint** served securely via CloudFront + S3 🎉
...
Next → **Part 2**: backend infrastructure (App Runner / API Gateway + Lambda + WebSocket integration)
