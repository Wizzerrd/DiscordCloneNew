...
export async function buildDatabaseURL() {
    ...
    const dbName = "postgres";  // your actual database name
    const port = 5432;
    ...
    const url = `postgresql://${encodeURIComponent(secret.username)}:${encodeURIComponent(secret.password)}@${host}:${port}/${dbName}?sslmode=require`;
